...
export class SpinService {
  ...
  async spin(
    campaignId: string,
    customerIdentifier: string | { email?: string; phone?: string },
  ) {
    // 0. Extract customer identifiers
    ...
    if (typeof customerIdentifier === 'string') {
      ...
      if (customerIdentifier.email) {
        customerEmail = customerIdentifier.email.trim().toLowerCase();
      }
      if (customerIdentifier.phone) {
        if (customerIdentifier.phone.includes('@') && !customerEmail) {
          customerEmail = customerIdentifier.phone.trim().toLowerCase();
        } else {
          customerPhone = customerIdentifier.phone.trim();
        }
      }
    }

    if (!customerEmail && !customerPhone) {
      throw new BadRequestException('Customer email or identifier is required to spin the wheel');
    }
    ...
    if (customerEmail) {
      identityConditions.push({ customerEmail });
      identityConditions.push({ customerPhone: customerEmail });
    }
    if (customerPhone) {
      identityConditions.push({ customerPhone });
    }

    const existingSpin = await this.spinRecordModel
      .findOne({
        campaignId: resolvedCampaignId,
        $or: identityConditions.length > 0 ? identityConditions : [{ customerEmail: 'unknown' }],
      })
      .exec();
    ...
    // 10. Send Winning Coupon Offer Notification Email via Resend
    // (SMS code is hidden / commented out per request)
    /*
    this.smsService
    ...
    */
    ...
    const recipientEmail = customerEmail || (customerPhone && customerPhone.includes('@') ? customerPhone : undefined);
    if (recipientEmail) {
      this.emailService
        .sendOfferEmail(
          recipientEmail,
          ...
          coupon.code,
          VALIDITY_DAYS,
          winningOffer.description,
          winningOffer.terms,
        )
        .then((result) => {
          this.logger.log(
            `Offer notification email dispatched to ${recipientEmail}: ${
        ...
        .catch((err) => {
          this.logger.error(
            `Failed to send offer notification email to ${recipientEmail}: ${err.message}`,
            err.stack,
          );
        });
    }
  ...
  async findAllSpins(
    filters: { campaignId?: string; customerPhone?: string; customerEmail?: string } = {},
  ): Promise<SpinRecordDocument[]> {
    const query: any = {};
